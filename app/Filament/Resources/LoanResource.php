                ...
                Forms\Components\Section::make('Course Details')->schema([
                    Wizard::make([
                        ...
                        ->schema([
                            ...
                            Forms\Components\TextInput::make('tuition_fees')->numeric()->required()->debounce(1000),
                            Forms\Components\TextInput::make('hostel_fees')->numeric()->required()->debounce(1000),
                            Forms\Components\TextInput::make('other_costs')->numeric()->required()->debounce(1000),
                            Forms\Components\TextInput::make('cost_of_living')->numeric()->required()->debounce(1000),
                        ])->columns(columns:2)->reactive()
                        ...
                        //     //if ($livewire->getCurrentStep() === 'amount') {
                        //         try {
                        //             $tuition_fees = floatval($livewire->form->getState()['tuition_fees'] ?? 0);
                        //             $hostel_fees = floatval($livewire->form->getState()['hostel_fees'] ?? 0);
                        //             $other_costs = floatval($livewire->form->getState()['other_costs'] ?? 0);
                        //             $cost_of_living = floatval($livewire->form->getState()['cost_of_living'] ?? 0);

                        //             $totalLoanAmount = $tuition_fees + $hostel_fees + $other_costs + $cost_of_living;
                        //             $livewire->form->fill(['total_loan_amount' => $totalLoanAmount]);

                        //             logger()->info('Total loan amount calculated: ', ['amount' => $totalLoanAmount]);
                        //         } catch (\Exception $e) {
                        //             logger()->error('Error in afterValidation: ' . $e->getMessage());
                        //             throw $e; // Rethrow the exception to make it visible
                        //         }
                        //     //}
                        // }),
